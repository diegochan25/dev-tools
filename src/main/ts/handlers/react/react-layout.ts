...
export class ReactLayout {
    ...
    public static async action(args: Map<string, Primitive>): Promise<void> {
        ...
        if (path.basename(workdir.abspath) !== path.basename(inputpath)) {
            name = await UI.ask("Enter a name for the layout:");
        }
        ...
        const contents = new Template(strings.TEMPLATE_PATH, template.template)
            .pass({
                names: names,
                useTypescript: lang === ReactLanguageRule.TS || lang === ReactLanguageRule.TSX,
                useStyles: true,
        ...
        UI.success("Layout '%s' has been created!", name).exit(0);
    }
